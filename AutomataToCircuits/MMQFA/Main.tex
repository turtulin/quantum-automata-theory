...
\begin{algorithm}[H]
  ...
  \For{$i = 1$ \textbf{to} $L$}
     ...
     \State Apply $U_{x_i}$ to current state $\ket{\psi_{i-1}}$
     ...
     \]
     \State $p \gets p + p_{\text{acc}}^{(i)}$
     \State Collapse $\ket{\psi_i}$ to $\frac{P_{\text{cont}} \ket{\psi_i}}{\|P_{\text{cont}} \ket{\psi_i}\|}$ if $p_{\text{cont}}^{(i)} > 0$
     \If{$p_{\text{cont}}^{(i)} = 0$}
