...
\glspl{fa} stand at the very origin of algorithmic language processing and they form the canonical recognisers of the \glspl{reg}, a class originally isolated by Kleene through rational expressions \cite{Kleene1956}. Their mathematical roots trace back to Chomsky's hierarchy of grammars \cite{Chomsky1959} and to the seminal definition of machine based language recognition given independently by Rabin and Scott \cite{RabinScott1959}. The structural essence of regularity was soon clarified by the Myhill and Nerode equivalence relation, which provides a necessary and sufficient criterion for finite recognisability without reference to a particular device \cite{Myhill1957,Nerode1958}. These results, later unified in modern textbooks \cite{HopcroftUllman1979,AhoHopcroftUllman1974,Sipser2012}, still underpin today's compiler front-ends, hardware verification workflows and network protocol design.

Historically four computational patterns crystallised out of the study of \glspl{cfa}. The first is the \gls{dfa}, a fully deterministic device that admits a single computation path and thus offers linear time membership testing together with minimal state equivalence \cite{HopcroftUllman1979}. The second is the \gls{nfa}, which explores many futures in parallel and enjoys an exponential succinctness advantage while remaining equivalent in expressive power to the deterministic model \cite{RabinScott1959}. Probabilistic branching leads to the \gls{pfa}, an automaton that attaches transition weights and decides membership relative to a rational cut point; with an isolated threshold it still recognises only \glspl{reg} \cite{Paz1971} whereas an arbitrary threshold allows stochastic languages that are not regular \cite{Turakainen1969}. Finally lifting the read head restriction yields two-way variants that may revisit earlier symbols and thereby capture algorithms such as lexical scanners with look ahead, yet they do not surpass regular expressiveness \cite{Freivalds1982}.
...
A non empty finite set $\Sigma$ of symbols is called an alphabet \cite{HopcroftUllman1979}.  
\end{definition}
...
Concatenation $u\cdot v$ appends the sequence of $v$ to $u$ \cite{HopcroftUllman1979}.  
\end{definition}
...
$\Sigma^{\ast}$ under concatenation with identity $\varepsilon$ \cite{HopcroftUllman1979}.  
\end{notation}

\begin{definition}[Formal Language]\label{def:language}
Any subset $L\subseteq\Sigma^{\ast}$ is a language \cite{HopcroftUllman1979}.  
\end{definition}
...
A type~3 grammar in Chomsky's hierarchy~\ref{fig:chomsky-hierarchy} generates the \glspl{reg} \cite{Chomsky1959}.  
\end{concept}
...
$L_{1}\,L_{2}$ and $L_{1}^{\ast}$ \cite{HopcroftUllman1979}.  
\end{proposition}
...
admits finitely many equivalence classes \cite{HopcroftUllman1979}.  
\end{theorem}
...
is described by the regular expression $\Sigma^{\ast}1$ \cite{HopcroftUllman1979}.  
\end{example}
...
tokenisers, model checking and hardware controllers \cite{AhoHopcroftUllman1974}.  
\end{observation}
...
\cite{HopcroftUllman1979}.  This functional behaviour yields predictable
...
\cite{Moore1956}.  The Hopcroft partition refinement algorithm computes
$M_{\mathrm{min}}$ in $O(|Q|\log|Q|)$ time \cite{Hopcroft1971}.
\end{theorem}
...
$L(M_{1})\subseteq L(M_{2})$ \cite{HopcroftUllman1979}.
\end{proposition}
...
\cite{HopcroftUllman1979}.  The relation $\equiv_{L}$ has exactly two
...
three after each bit is read \cite{Sipser2012}.
\end{example}
...
\cite{AhoHopcroftUllman1974}.  Deterministic automata thus form the
...
word to allow silent moves \cite{RabinScott1959}.
\end{definition}
...
$S_{|w|}\cap F\neq\emptyset$ \cite{HopcroftUllman1979}.
\end{lemma}
...
\cite{HopcroftUllman1979}.  Subsequent sections compare these classical
...
$L=L(M,\lambda)$ \cite{Rabin1963}.  The family of all stochastic
...
universality and equivalence are undecidable \cite{Paz1971}.
\end{proposition}
...
\cite{Rabin1963}.  The pumping lemma shows that $L$ is not regular,
...
\cite{Rabin1963,Paz1971}.  Taken together, these results show that
...
\cite{Shepherdson1959,Freivalds1982}.
\end{theorem}
